...
type
  TAdrockHotKeyCustom = class(THotKey)
  ...
  TAdrockHotKey = class(TAdrockHotKeyCustom)
  published
    { Published declarations }
...
Constructor TAdrockHotKeyCustom.Create(Aowner : TComponent);
begin
...
Destructor TAdrockHotKeyCustom.Destroy;
begin
  fWhenFocused.Free;
  fWhenFocused := NIL;
  Inherited Destroy;
...
Procedure TAdrockHotKeyCustom.Loaded;
begin
  inherited Loaded;
end;

Procedure TAdrockHotKeyCustom.CMEnter(Var Message : TCMEnter);
begin
  Inherited;
  fWhenFocused.OnEnter;
end;

Procedure TAdrockHotKeyCustom.CMExit(Var Message : TCMExit);
begin
  fWhenFocused.OnExit;
  Inherited;
end;

Procedure TAdrockHotKeyCustom.BeforeChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeChange)) then
    fOnBeforeChange(Sender);
end;

Procedure TAdrockHotKeyCustom.BeforeDataChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeDataChange)) then
    fOnBeforeDataChange(Sender);
end;

Procedure TAdrockHotKeyCustom.CustomOnEnterWhenFocusedHandle(Sender : TObject; fOwner : TObject;
...
Procedure TAdrockHotKeyCustom.CustomOnExitWhenFocusedHandle(Sender : TObject; fOwner : TObject; fSavedBackColor : TColor);
begin
  Color := fSavedBackColor;
end;

procedure TAdrockHotKeyCustom.Notification(AComponent: TComponent;  Operation: TOperation);
begin
