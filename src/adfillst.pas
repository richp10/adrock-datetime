...
type
  TAdrockFileListBoxCustom = class(TFileListBox)
  ...
  TAdrockFileListBox = class(TAdrockFileListBoxCustom)
  published
    { Published declarations }
...
Constructor TAdrockFileListBoxCustom.Create(Aowner : TComponent);
begin
...
Destructor TAdrockFileListBoxCustom.Destroy;
begin
  fWhenFocused.Free;
  fWhenFocused := NIL;
  Inherited Destroy;
...
Procedure TAdrockFileListBoxCustom.Loaded;
begin
  inherited Loaded;
end;

Procedure TAdrockFileListBoxCustom.CMEnter(Var Message : TCMEnter);
begin
  Inherited;
  fWhenFocused.OnEnter;
end;

Procedure TAdrockFileListBoxCustom.CMExit(Var Message : TCMExit);
begin
  fWhenFocused.OnExit;
  Inherited;
end;

Procedure TAdrockFileListBoxCustom.BeforeChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeChange)) then
    fOnBeforeChange(Sender);
end;

Procedure TAdrockFileListBoxCustom.BeforeDataChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeDataChange)) then
    fOnBeforeDataChange(Sender);
end;

Procedure TAdrockFileListBoxCustom.CustomOnEnterWhenFocusedHandle(Sender : TObject; fOwner : TObject;
...
Procedure TAdrockFileListBoxCustom.CustomOnExitWhenFocusedHandle(Sender : TObject; fOwner : TObject;
  fSavedBackColor : TColor);
begin
  Color := fSavedBackColor;
end;

procedure TAdrockFileListBoxCustom.Notification(AComponent: TComponent;  Operation: TOperation);
begin
