...
type
  TAdrockListBoxCustom = class(TlistBox)
  ...
  TAdrockListBox = class(TAdrockListBoxCustom)
  published
    { Published declarations }
...
Constructor TAdrockListBoxCustom.Create(Aowner : TComponent);
begin
...
Destructor TAdrockListBoxCustom.Destroy;
begin
  fWhenFocused.Free;
  fWhenFocused := NIL;
  Inherited Destroy;
...
Procedure TAdrockListBoxCustom.Loaded;
begin
  inherited Loaded;
end;

Procedure TAdrockListBoxCustom.CMEnter(Var Message : TCMEnter);
begin
  Inherited;
  fWhenFocused.OnEnter;
end;

Procedure TAdrockListBoxCustom.CMExit(Var Message : TCMExit);
begin
  fWhenFocused.OnExit;
  Inherited;
end;

Procedure TAdrockListBoxCustom.BeforeChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeChange)) then
    fOnBeforeChange(Sender);
end;

Procedure TAdrockListBoxCustom.BeforeDataChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeDataChange)) then
    fOnBeforeDataChange(Sender);
end;

procedure TAdrockListBoxCustom.Notification(AComponent: TComponent;  Operation: TOperation);
begin
