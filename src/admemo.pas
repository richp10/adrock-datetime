...
uses
  ...
  TAdrockMemo = class(TAdrockMemoCustom)
  published
    { Published declarations }
...
Constructor TAdrockMemoCustom.Create(Aowner : TComponent);
begin
...
Destructor TAdrockMemoCustom.Destroy;
begin
  fWhenFocused.Free;
  fWhenFocused := NIL;
  Inherited Destroy;
...
Procedure TAdrockMemoCustom.Loaded;
begin
  inherited Loaded;
end;

Procedure TAdrockMemoCustom.CMEnter(Var Message : TCMEnter);
begin
  Inherited;
  fWhenFocused.OnEnter;
end;

Procedure TAdrockMemoCustom.CMExit(Var Message : TCMExit);
begin
  fWhenFocused.OnExit;
  Inherited;
end;

Procedure TAdrockMemoCustom.BeforeChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeChange)) then
    fOnBeforeChange(Sender);
end;

Procedure TAdrockMemoCustom.BeforeDataChange(Sender : TObject);
begin
  if (Assigned(fOnBeforeDataChange)) then
    fOnBeforeDataChange(Sender);
end;

procedure TAdrockMemoCustom.Notification(AComponent: TComponent;  Operation: TOperation);
begin
